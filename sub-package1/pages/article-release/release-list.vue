			...
			<ul>
				<li v-for="(item,index) in list" :key="index">
					<view class="centerBox">
						...
						<view class="stateBox">
							<view>
								<image v-if="item.reviewStatus==1" src="../../static/img/pass.png" mode="" class="icon">
								</image>
								<p v-if="item.reviewStatus==1">{{articleRelease['Approved']||'已通过审批'}}</p>

								<image v-if="item.reviewStatus==0" src="../../static/img/dd.png" mode="" class="icon">
								</image>
								<p v-if="item.reviewStatus==0">{{articleRelease['Approvalinprogress']||'正在审核'}}</p>

								<image v-if="item.reviewStatus==2" src="../../static/img/nopass.png" mode="" class="icon">
								</image>
