	...
	<uni-base-page class="mytalk">
		...
		<ul>
			<li class="clear" v-for="(item, index) in talkList" :key="index" @click="comment(item.id)">
				...
				<i class="delete" @click.stop="deltalk(item.id)"><image src="../../static/img/talkdelete.png" /></i>
			...
			<movable-view :x="x" :y="y" direction="all" @change="onChange">
				<image @click="goTotalk" class="send" src="../../static/img/sendicon.png"></image>
			</movable-view>
		</movable-area>
	</uni-base-page>
</template>
...
export default {
	...
	methods: {
		...
		goTotalk() {
			uni.navigateTo({
				url: "../../../sub-package1/pages/talking/talking?originatePage=1"
			});
		},
		// 说说点赞
		talkLike(id,item){
