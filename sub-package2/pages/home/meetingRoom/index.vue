          ...
          <u-dropdown-item
            ...
            <view class="content-box">
              <view class="slot-content" style="background-color: #ffffff">
                <view class="">
                  <view class="dropdown_title"> 容纳人数区间 </view>
      ...
      <view class="card-box">
        <view class="card" v-for="(item, index) in dataList" :key="item.id" @click="goMeetingDetails(item)">
          <view class="card-left">
            <u-image style="margin-right: 20rpx" :fade="false" width="180" height="160" :src="item.images[0]"></u-image>
          </view>
          <view class="card-center">
            <view class="card-center-title">
              {{ item.ownedBuilding + "/" + item.ownedFloor + "/" + item.name }}
            </view>
            <view class="card-center-content"> 可容纳：{{ item.minimumOccupancy }}-{{ item.maximumCapacity }}人 </view>
          </view>
