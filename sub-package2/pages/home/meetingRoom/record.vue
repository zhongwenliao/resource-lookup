<template>
  <page>
        ...
        <view class="card" v-for="(item, index) in dataList" :key="item.id">
          <view class="card-head">
            <u-icon
              ...
              label-color="#333333"
              name="dizhi"
              :label="item.room.buildingName + item.room.ownedFloor + item.room.name"
              custom-prefix="custom-icon"
              size="33"
            ></u-icon>
            <view v-if="item.approvalStatus === 0" class="card-head-right" @click="openModal(item)"> 取消预约 </view>
          </view>
          <u-line color="#F1F2F3" />
          <view class="card-body" @click="review(item)">
            <view class="card-body-box">
              <view class="card-body-title"> 联系人： </view>
              <view class="card-body-content">
                {{ item.contact + "/" + item.phone }}
              </view>
            </view>
            <view class="card-body-box" v-if="enterpriseId > 0">
              <view class="card-body-title"> 企业名称： </view>
              <view class="card-body-content">
            ...
            <view class="card-body-box" v-if="item.approvalStatus !== 0">
              <view class="card-body-title"> 审批人： </view>
              <view class="card-body-content">
                {{ item.updateBy || "-" }}
              </view>
            </view>
            <view class="card-body-box">
              <view class="card-body-title"> 会议时间： </view>
    ...
    <u-modal
      ...
      :content-style="{ textAlign: 'center', margin: '30upx' }"
      :show-cancel-button="true"
      confirm-text="确定"
      @confirm="modelConfirm"
    >
      <view class="slot-content"> 取消预约后，系统将撤回预约申请， 确定取消预约吗？ </view>
    </u-modal>
    <u-toast ref="uToast" />
  </page>
</template>
...
export default {
      ...
      meetingRoomTabbar: [
        {
          ...
          iconPath: "https://file.yuanzhoulvwego.com/prod/uploadFiles/home/meetingRoom/huiyishiyuyue2.png",
          text: "会议室预约",
        },
        {
          pagePath: "/sub-package2/pages/home/meetingRoom/record",
          iconPath: "https://file.yuanzhoulvwego.com/prod/uploadFiles/home/meetingRoom/yuyuejilu.png",
          text: "预约记录",
        },
      ],
      // 顶部分类
  ...
  methods: {
    ...
    getInfo(pageNo, pageSize) {
      ...
      this.$api("meetingRoom.getMeetingRoomlist", params)
        .then((res) => {
          if (res.code == 200) {
