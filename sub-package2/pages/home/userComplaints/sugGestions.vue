<template>
	<view class="body">
			...
			<u-form :border-bottom="false" label-width="150upx" :model="form" ref="uForm">
				...
				<view class="form-item_box">
					...
					<view class="form-item_box_body">
						...
						<u-form-item required class="form-item" prop="classificationName" :border-bottom="false"
							label="反馈类型">
							<u-input type="select" @click="repairTypeSelect = true" placeholder="请选择反馈类型"
				...
				<view class="form-item_box">
					<view class="form-item_box_head">
						<span class="form-item_box_head_text">反馈问题</span>
					</view>
					<view class="form-item_box_body">
						<u-form-item label-position="top" :border-bottom="false" label="">
							<my-textarea :value.sync="form.repairDescription" :maxlength="100" :placeholder="'请输入反馈问题'"></my-textarea>
						</u-form-item>
					</view>
				</view>
				<!-- 投诉问题 end -->
				...
				<view class="form-item_box">
					...
					<view class="form-item_box_body">
						<u-form-item label-position="top" prop="" :label-style="labelStyle" :border-bottom="false"
							label="">
		...
		<!-- 投诉类型选择 -->
		<u-select @confirm="confirm" value-name="id" label-name="name" v-model="repairTypeSelect"
	...
	export default {
		data() {
			return {
				...
				rules: {
					...
					classificationName: [{ required: true, message: '请选择投诉类型', trigger: ['change'] }]
				}
			}
		},
		onReady() {
		...
		methods: {
			...
			getAllMaintainClassifyList() {
				...
				this.$api('userComplaints.getComplaintsALL', params, false).then(res => {
					if (res.code == 200) {
			...
			onSubmit() {
				this.$refs.uForm.validate(valid => {
					if (valid) {
						if (this.form.repairDescription) {
							...
							console.log('opopop')
							this.$api('userComplaints.getComplaints', params, false).then(res => {
								if (res.code === 200) {
									this.showModel = true
								} else {
