		...
		<view class="repair-progress">
			...
			<view class="repair-progress-conter">
				...
				<view class="form-item_box">
					...
					<view class="form-item_box_body">
						<view class="form-item_box_body_text">{{ ruleForm.repairDescription || '暂无' }}</view>
					</view>
				</view>
				<!-- 投诉问题 end -->
				<view class="form-item_box">
					<view class="form-item_box_head">
						<span class="form-item_box_head_text">问题处理情况反馈</span>
					</view>
					<view class="form-item_box_body">
						<view class="form-item_box_body_text">{{ ruleForm.commentDescription || '暂无' }}</view>
					</view>
				</view>
				<!-- 投诉照片 start -->
				<view class="form-item_box">
					<view class="form-item_box_head">
						<span class="form-item_box_head_text">投诉照片</span>
					</view>
					<view class="form-item_box_body">
						<u-row gutter="16" justify="between">
							<u-col span="4" v-for="(item, index) in ruleForm.livePhoto" :key="index">
				...
				<!-- 投诉照片 end -->
				<view class="form-item_box" v-if="ruleForm.processingStatus === 4 && ruleForm.evaluationStatus === 1 ">
					<view class="form-item_box_head">
						<text class="form-item_box_head_text" >我的评价</text>
					</view>
					<view class="form-item_box_body">
						<view class="form-item_box_body_text">
							{{ruleForm.suggestions}}
						</view>
						
						<view style="margin-left: 28upx;">评价结果 ：
							<u-rate
	...
	export default {
		...
		methods: {
			getComplaintsByld(id) {
				let params = { id: id }
				this.$api('userComplaints.getComplaintsByld', params).then(res => {
					if (res.code == 200) {
