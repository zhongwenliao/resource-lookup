              ...
              <view v-for="(item, index) in list" :key="index" style="width: 100%" class="">
                <view class="u-m-b-20">
                  <u-icon
                    ...
                    @click="showOrHide(item)"></u-icon>
                  <u-checkbox
                    @change="checkboxChange"
                    v-model="item.checked"
                    :name="item.name"
                    :active-color="
                      item.childMenu.filter((ccitem) => {
                        return ccitem.checked == true;
                      }).length === item.childMenu.length
                        ? '#2979FF'
                        : '#FF9900'
                    ">
                    <view>{{ item.name }}</view>
...
export default {
  ...
  methods: {
    ...
    roleMenuList() {
      ...
      this.$api("employeeManagement.roleMenuList", { menuType: 2, zoneId: this.zoneId })
        .then((res) => {
          if (res.code == 200) {
        ...
        if (valid) {
          if (_this.checkList.length > 0) {
            ...
            if (_this.id > 0) {
              ...
              // 新增部门
              _this
                .$api("employeeManagement.addDepartment", params, false)
                .then((res) => {
                  if (res.code == 200) {
                    _this.$refs.uToast.show({
                      title: res.msg
