...
(function(layui, window, factory) {
  ...
  Common.prototype.init = function(target) {
    ...
    $(target ? target : `select[${NAME}]`).each((index, select) => {
      let othis = $(select),
        ...
        value = othis.find("option[selected]").toArray().map(option => {
          //获取已选中的数据
          return {
      ...
      let inputHtml = [
        `<div class="${LABEL}">`,
        `<input type="text" fsw class="${FORM_INPUT} ${INPUT}" ${fs.config.isSearch
          ? ""
          : 'style="display: none;"'} autocomplete="off" debounce="0" />`,
        `</div>`
      ];
      let reElem = $(`<div class="${FORM_SELECT}" ${SKIN}="${fs.config.skin}">
					<input class="${HIDE_INPUT}" value="" name="${fs.config.formname}" lay-verify="${fs.config.layverify}" lay-verType="${fs
        .config
        .layverType}" type="text" style="position: absolute;bottom: 0; z-index: -1;width: 100%; height: 100%; border: none; opacity: 0;"/>
					<div class="${FORM_TITLE} ${fs.config.disabled ? DIS : ""}">
						...
						<div class="${TDIV}">
							<input type="text" autocomplete="off" placeholder="${fs.config
                .placeholder}" readonly="readonly" unselectable="on" class="${FORM_INPUT}">
						</div>
						<div></div>
					</div>
    ...
    (div ? [div] : $(`.${FORM_SELECT}`).toArray()).forEach((reElem, index) => {
      ...
      if ((id && data[id] && data[id].config.isEmpty) || isCall) {
        this.search(
          ...
          null,
          null,
          data[id].config.searchType == 0 ? reElem.find(`.${LABEL} .${INPUT}`) : reElem.find(`dl .${FORM_DL_INPUT} .${INPUT}`)
        );
      }
    });
  };
  ...
  Common.prototype.renderLinkage = function(id, dataArr, linkageWidth) {
    ...
    $.each(result, (idx, arr) => {
      let groupDiv = [
        `<div style="left: ${(linkageWidth - 0) * idx}px;" class="xm-select-linkage-group xm-select-linkage-group${idx +
          1} ${idx != 0 ? "xm-select-linkage-hide" : ""}">`
      ];
      $.each(arr, (idx2, item) => {
  ...
  Common.prototype.createQuickBtn = function(obj, right) {
    return `<div class="${CZ}" method="${obj.name}" title="${obj.name}" ${right
          ...
          len
            ...
            : (
                ...
                (childrens = childrens.filter(
                  (index, item) => $(item).attr("tree-id").split("-").length - 1 == treeId.split("-").length
                ))
              );
          childrens.animate(
            {
  ...
  Common.prototype.setHidnVal = function(key, label) {
    ...
    label.parents(`.${PNAME}`).find(`.${HIDE_INPUT}`).val(
      data[key].values
        .map(val => {
          return val.value;
        })
  ...
  Common.prototype.handlerLabel = function(id, dd, isAdd, oval, notOn) {
    ...
    isAdd
      ? (
          dd && dd[0] ? (dd.addClass(THIS), dd.removeClass(TEMP)) : dl.find(".xm-select-linkage")[0] && this.linkageAdd(id, val),
          this.addLabel(id, div, val),
          vals.push(val)
        )
      : (
          dd && dd[0] ? dd.removeClass(THIS) : dl.find(".xm-select-linkage")[0] && this.linkageDel(id, val),
  ...
  Common.prototype.selectAll = function(id, isOn, skipDis) {
    ...
    dl
      .find(`dd[lay-value]:not(.${FORM_SELECT_TIPS}):not(.${THIS})${skipDis ? ":not(." + DISABLED + ")" : ""}`)
      .each((index, item) => {
        item = $(item);
