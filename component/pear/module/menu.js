layui.define(["table", "jquery", "element"], function(exports) {
    ...
    setTimeout(function() {
      $("#" + opt.control + " .control").on("mousewheel DOMMouseScroll", function(event) {
        ...
        if (delta > 0) {
          ...
          if (
            Number($("#" + opt.control + " .control ul").css("left").replace("px", "")) +
              ($("#" + opt.control + " .control ul").width() - $("#" + opt.control + " .control").width()) >
            0
          ) {
            for (var num = 1; num < 20; num++) {
              setTimeout(function() {
  ...
  pearMenu.prototype.collapse = function(time) {
    ...
    if ($("#" + this.option.elem).is(".pear-nav-mini")) {
      ...
      var that = this;
      $("#" + this.option.elem).promise().done(function() {
    ...
    $.each(option.data, function(i, item) {
      ...
      if (i === option.defaultMenu) {
        controlItem =
          ...
          "</a></li>";
        menuItem =
          '<ul  pear-id="' + item.id + '" lay-filter="' + option.elem + '" class="layui-nav arrow layui-nav-tree pear-nav-tree">';
  ...
  function isHoverMenu(b, option) {
    if (b) {
      ...
      $(navItem + "," + navChildDd).mouseenter(function() {
        var _this = $(this);
        _this.siblings().find(".layui-nav-child").removeClass("layui-nav-hover").css({
          left: 0,
          top: 0
        });
      ...
      $(navItem + "," + navChildDl).mouseleave(function() {
        var _this = $(this);
        _this.closest(".layui-nav-item").find(".layui-nav-child").removeClass("layui-nav-hover").css({
          left: 0,
          top: 0
        });
      });
