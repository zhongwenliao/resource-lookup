layui.define(["table", "jquery", "element"], function(exports) {
  ...
  var pearOper = new function() {
    ...
    this.block = function(option) {
      if (option.type == 1) {
        Notiflix.Block.Standard(option.elem, option.msg);
      } else if (option.type == 2) {
        Notiflix.Block.Hourglass(option.elem, option.msg);
      } else if (option.type == 3) {
        Notiflix.Block.Circle(option.elem, option.msg);
      } else if (option.type == 4) {
        Notiflix.Block.Arrows(option.elem, option.msg);
      } else if (option.type == 5) {
        Notiflix.Block.Dots(option.elem, option.msg);
      } else if (option.type == 6) {
        Notiflix.Block.Pulse(option.elem, option.msg);
      }
    };
    this.report = function(option) {
      if (option.type == "success") {
        Notiflix.Report.Success(option.title, option.message, "确认");
      } else if (option.type == "failure") {
        Notiflix.Report.Failure(option.title, option.message, "确认");
      } else if (option.type == "warning") {
        Notiflix.Report.Warning(option.title, option.message, "确认");
      } else if (option.type == "info") {
        Notiflix.Report.Info(option.title, option.message, "确认");
      }
    };
    ...
    this.confirm = function(option) {
      Notiflix.Confirm.Show(option.title, option.message, "确认", "取消", option.success, option.cancle);
    };
  ...
  var notiflixGoogleFont = function(use, family) {
    if (
      !document.getElementById("NotiflixQuicksand") &&
      use &&
      (family && typeof family === "string" && family.toLowerCase() === "quicksand")
    ) {
  ...
  var NotiflixNotify = function(message, callback, theType, staticType) {
    if (arguments && arguments.length === 4) {
      ...
      if (newNotifySettings.backOverlay) {
        ...
        notifyOverlay.style.animationDuration = newNotifySettings.cssAnimation
      ...
      ntflxNotify.className =
        ...
        " " +
        (newNotifySettings.closeButton && !callback ? "with-close-button" : "") +
        " " +
        (callback && typeof callback === "function" ? "with-callback" : "") +
        " " +
        (newNotifySettings.clickToClose ? "click-to-close" : "");
      ...
      if (newNotifySettings.useIcon) {
        ...
        if (newNotifySettings.useFontAwesome) {
          ntflxNotify.innerHTML =
            ...
            " " +
            (newNotifySettings.fontAwesomeIconStyle === "shadow" ? "shadow" : "basic") +
        ...
        // without icon
        ntflxNotify.innerHTML =
          '<span class="the-message">' + message + "</span>" + (newNotifySettings.closeButton ? closeButtonHTML : "");
      }
      // use icon off
      // notify content off
      ...
      if (newNotifySettings.useIcon) {
        // if useIcon, dynamically vertical align the contents
      ...
      if (document.getElementById(ntflxNotify.id)) {
        ...
        var removeNotifyElmentsAndWrapper = function() {
          ...
          if (removeWrap.childElementCount <= 0 && removeWrap.parentNode !== null) {
      ...
      if (newNotifySettings.showOnlyTheLastOne && notiflixNotifyCount > 0) {
        var allNotifyElmNotTheLastOne = document.querySelectorAll(
          "[id^=" + newNotifySettings.ID + "-]:not([id=" + newNotifySettings.ID + "-" + notiflixNotifyCount + "])"
        );
  ...
  var NotiflixConfirm = function(title, message, okButtonText, cancelButtonText, okButtonCallback, cancelButtonCallback) {
    ...
    ntflxConfirmWrap.className =
      ...
      (newConfirmSettings.cssAnimation ? " with-animation nx-" + newConfirmSettings.cssAnimationStyle : "");
    ...
    ntflxConfirmWrap.innerHTML =
      ...
      newConfirmSettings.className +
      '-buttons">' +
      '<a id="NXConfirmButtonOk" class="confirm-button-ok' +
      (okButtonCallback ? "" : " full") +
      '" style="color:' +
    ...
    if (!document.getElementById(ntflxConfirmWrap.id)) {
      ...
      // position on
      if (newConfirmSettings.position === "center") {
        // if center-center
        var windowH = Math.round(window.innerHeight);
      ...
      } else if (newConfirmSettings.position === "right-top") {
        // if right-top
        ntflxConfirmWrap.style.right = newConfirmSettings.distance;
        ...
        ntflxConfirmWrap.style.margin = "auto";
      } else if (newConfirmSettings.position === "right-bottom") {
        // if right-bottom
        ntflxConfirmWrap.style.right = newConfirmSettings.distance;
        ...
        ntflxConfirmWrap.style.left = "auto";
        ntflxConfirmWrap.style.margin = "auto";
      } else if (newConfirmSettings.position === "left-top") {
        // if left-top
        ntflxConfirmWrap.style.left = newConfirmSettings.distance;
        ntflxConfirmWrap.style.top = newConfirmSettings.distance;
        ...
        ntflxConfirmWrap.style.bottom = "auto";
        ntflxConfirmWrap.style.margin = "auto";
      } else if (newConfirmSettings.position === "left-bottom") {
        // if left-bottom
        ntflxConfirmWrap.style.left = newConfirmSettings.distance;
        ntflxConfirmWrap.style.bottom = newConfirmSettings.distance;
        ntflxConfirmWrap.style.top = "auto";
    ...
    if (getSelector.length > 0) {
      ...
      notiflixConsoleError(
        ...
        'You called the "Notiflix.Block..." function with "' +
          selector +
          '" selector, but there is no such element(s) on the document.'
      );
      return false;
    }
        ...
        if (eachBlockElement.length < 1) {
          ...
          if (positions.indexOf(eachSelectorPos) <= -1) {
            ...
            var style =
              ...
              notiflixBlockElementCounter +
              '">' +
              eachSelectorIdOrClass +
              "." +
              blockClassName +
              "-position{position:relative!important;min-height:" +
              minHeight +
              ";}" +
              "</style>";
