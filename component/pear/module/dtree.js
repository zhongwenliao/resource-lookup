layui.define(["jquery", "layer", "form"], function(exports) {
  ...
  // 树的公共定义样式汇总
  var LI_NAV_CHILD = "dtree-nav-ul-sid",
    LI_NAV_ITEM = "dtree-nav-item",
    LI_DIV_ITEM = "dtree-nav-div",
    DTREEFONTSPECIAL = "dtreefont-special",
    NONETITLE = "dtree-none-text",
    LI_DIV_MENUBAR = "dtree-menubar",
    LI_DIV_TOOLBAR = "dtree-toolbar",
    TOOLBAR_TOOL = "dtree-toolbar-tool",
    TOOLBAR_TOOL_EM = "dtree-toolbar-fixed",
    LI_DIV_CHECKBAR = "dtree-nav-checkbox-div",
    LI_CLICK_CHECKBAR = "d-click-checkbar", //绑定点击复选框时需要用到
    LI_DIV_TEXT_CLASS = "t-click",
    UL_ROOT = "dtree",
    LI_NAV_FIRST_LINE = "dtree-nav-first-line",
    LI_NAV_LINE = "dtree-nav-line",
  ...
  var AjaxHelper = {
    ...
    serialize: function(first, param) {
      //json序列化   key=value&key1=value1
      var p = first;
  ...
  var DTree = function(options) {
    ...
    this.toolbarFun = {
      addTreeNode: function(param, $div) {
        //添加树节点后调用的函数，用于用户自定义，如未指定则树不会发生变化
        return;
      },
      editTreeNode: function(param, $div) {
        //编辑树节点后调用的函数，用于用户自定义，如未指定则树不会发生变化
        return;
      },
      editTreeLoad: function(param) {
        // 编辑树的数据回显，用于打开编辑时，回填数据
        return;
      },
      delTreeNode: function(param, $div) {
        //删除树后调用的函数，用于用户自定义，如未指定则树不会发生变化
        return;
      },
    ...
    this.menubarFun = {
      remove: function(checkbarNodes) {
        //删除复选框选中节点，需要用户自定义，如未指定则树只是页面上做了修改
        return true;
      }
    };
    this.menubarTips = {
      ...
      toolbar: [],
      group: [
        defaultMenu.moveDown,
        defaultMenu.moveUp,
        defaultMenu.refresh,
        defaultMenu.checkAll,
        defaultMenu.unCheckAll,
        defaultMenu.invertAll,
        defaultMenu.remove,
        defaultMenu.searchNode
      ],
      freedom: []
    };
    this.checkbarFun = {
      ...
      chooseDone: function(checkbarNodesParam) {
        //复选框点击事件完毕后，返回该树关于复选框操作的全部信息，用于用户自定义，如未指定则树只是页面上做了修改
        return;
      }
    };
    this.iframeDefaultRequest = {
      //iframe的默认参数,目的是与加载树的参数不一样
      nodeId: "nodeId", //节点ID
      parentId: "parentId", //父节点ID
      context: "context", //节点内容
      leaf: "leaf", //是否叶子节点
    ...
    this.iframeFun = {
      iframeDone: function(iframeParam) {
        //iframe加载完毕后，用于用户自定义事件
        return;
      }
    };
    this.style = {
  ...
  DTree.prototype.setting = function(options) {
    ...
    this.accordion =
      typeof this.options.accordion === "boolean"
        ? this.options.accordion
        : typeof OPTIONS.accordion === "boolean" ? OPTIONS.accordion : false; //手风琴加载， 默认false
    if (this.accordion) {
    ...
    this.cache =
      typeof this.options.cache === "boolean" ? this.options.cache : typeof OPTIONS.cache === "boolean" ? OPTIONS.cache : true; //数据缓存，默认true
    this.record = this.options.record || OPTIONS.record || false; //开启数据记录模式，默认false
    this.load =
      typeof this.options.load === "boolean" ? this.options.load : typeof OPTIONS.load === "boolean" ? OPTIONS.load : true; //开启加载动画，默认true
    ...
    if (this.skin == "layui") {
      // layui主题
      this.line =
        typeof this.options.line === "boolean" ? this.options.line : typeof OPTIONS.line === "boolean" ? OPTIONS.line : true; //开启树线，默认开启
      ...
      this.fnodeIcon =
        typeof this.ficon === "string" || typeof this.ficon === "number" ? (this.ficon == "-1" ? "-1" : "1") : this.ficon[0]; //一级图标中的node节点图标。0：文件夹，1：+、-，2：三角形， -1：不显示一级图标，默认'1'
      ...
      this.nodeIcon =
        typeof this.icon === "string" || typeof this.icon === "number" ? (this.icon == "-1" ? "-1" : "-1") : this.icon[0]; //二级图标中的node节点图标。0：文件夹，1：+、-，2：三角形， -1：不显示二级图标，默认'-1'
      this.leafIcon = typeof this.icon === "string" || typeof this.icon === "number" ? this.icon : this.icon[1]; //二级图标中的leaf节点图标
    } else if (this.skin == "laySimple") {
      // laySimple主题
      ...
      this.fnodeIcon =
        typeof this.ficon === "string" || typeof this.ficon === "number" ? (this.ficon == "-1" ? "-1" : "2") : this.ficon[0]; //一级图标中的node节点图标。0：文件夹，1：+、-，2：三角形， -1：不显示一级图标，默认'2'
      this.fleafIcon = typeof this.ficon === "string" || typeof this.ficon === "number" ? this.ficon : this.ficon[1]; // 一级图标中的leaf节点图标
      this.icon = this.options.icon || OPTIONS.icon || "-1"; //二级图标样式，0：文件夹，1：人员，2：机构，3：报表，4：信息，5：叶子，6：勋章，7：文件，8：小圆点，9：齿轮，10：星星， -1：不显示二级图标。默认'-1'
      this.nodeIcon =
        typeof this.icon === "string" || typeof this.icon === "number" ? (this.icon == "-1" ? "-1" : "-1") : this.icon[0]; //二级图标中的node节点图标。0：文件夹，1：+、-，2：三角形， -1：不显示二级图标，默认'-1'
      ...
      this.nodeIcon =
        typeof this.icon === "string" || typeof this.icon === "number" ? (this.icon == "-1" ? "-1" : "0") : this.icon[0]; //二级图标中的node节点图标。0：文件夹，1：+、-，2：三角形， -1：不显示二级图标，默认'0'
      this.leafIcon = typeof this.icon === "string" || typeof this.icon === "number" ? this.icon : this.icon[1]; //二级图标中的leaf节点图标
    }
    ...
    this.async =
      typeof this.options.async === "boolean" ? this.options.async : typeof OPTIONS.async === "boolean" ? OPTIONS.async : true; //异步同步加载,默认异步加载
    ...
    this.defaultRequest =
      $.extend(this.defaultRequest, this.options.defaultRequest || OPTIONS.defaultRequest) || this.defaultRequest; //默认请求参数
    ...
    this.beforeSend =
      ...
      OPTIONS.beforeSend ||
      function(ajax, XMLHttpRequest, self) {
        return true;
      }; //异步加载之前的回调
    ...
    this.iframeDefaultRequest =
      $.extend(this.iframeDefaultRequest, this.options.iframeDefaultRequest || OPTIONS.iframeDefaultRequest) ||
      this.iframeDefaultRequest; //iframe的默认传递参数
    this.iframeRequest =
      $.extend(this.iframeRequest, this.options.iframeRequest) ||
      $.extend(this.iframeRequest, OPTIONS.iframeRequest) ||
      this.iframeRequest; //iframe的自定义参数
    this.iframeFun =
      $.extend(this.iframeFun, this.options.iframeFun) || $.extend(this.iframeFun, OPTIONS.iframeFun) || this.iframeFun; //iframe事件加载
  ...
  DTree.prototype.reloadSetting = function(options) {
    ...
    if (this.skin == "layui") {
      ...
      this.fnodeIcon =
        typeof this.ficon === "string" || typeof this.ficon === "number" ? (this.ficon == "-1" ? "-1" : "1") : this.ficon[0]; //一级图标中的node节点图标。0：文件夹，1：+、-，2：三角形， -1：不显示二级图标，默认'1'
      this.fleafIcon = typeof this.ficon === "string" || typeof this.ficon === "number" ? this.ficon : this.ficon[1]; //一级图标中的leaf节点图标
      ...
      this.fnodeIcon =
        typeof this.ficon === "string" || typeof this.ficon === "number" ? (this.ficon == "-1" ? "-1" : "2") : this.ficon[0]; //一级图标中的node节点图标。0：文件夹，1：+、-，2：三角形， -1：不显示二级图标，默认'2'
      this.fleafIcon = typeof this.ficon === "string" || typeof this.ficon === "number" ? this.ficon : this.ficon[1]; //一级图标中的leaf节点图标
      this.icon = this.options.icon || this.icon; //二级图标样式，0：文件夹，1：人员，2：机构，3：报表，4：信息，5：叶子，6：勋章，7：文件，8：小圆点，9：齿轮，10：星星， -1：不显示二级图标。默认'-1'
      this.nodeIcon =
        typeof this.icon === "string" || typeof this.icon === "number" ? (this.icon == "-1" ? "-1" : "-1") : this.icon[0]; //二级图标中的node节点图标。0：文件夹，1：+、-，2：三角形， -1：不显示二级图标，默认'-1'
    ...
    this.withCredentials =
      typeof this.options.withCredentials === "boolean" ? this.options.withCredentials : this.withCredentials; //是否允许跨域请求
    ...
    this.iframeDefaultRequest =
      $.extend(this.iframeDefaultRequest, this.options.iframeDefaultRequest) || this.iframeDefaultRequest; //iframe的默认传递参数
  ...
  DTree.prototype.renderSelectDom = function() {
    ...
    // 设置html
    var prevHtml = [
      '<div class="layui-unselect layui-form-select" dtree-id="' + rootId + '" dtree-select="' + _this.selectDiv + '">',
      '<div class="layui-select-title">',
      selectInput.join(""),
      '<input type="text" dtree-id="' +
        rootId +
        '" id="' +
        _this.selectTipsName +
        '_id" name="' +
        _this.selectTipsName +
        '" placeholder="' +
        _this.selectTips +
        '" value="" readonly class="layui-input layui-unselect">',
    ...
    _this.obj
      .wrap(
        '<div class="layui-card dtree-select" dtree-id="' +
          rootId +
          '" dtree-card="' +
          _this.selectCardDiv +
          '" ' +
      ...
      .wrap('<div class="layui-card-body"></div>')
      .wrap('<div id="' + _this.selectTreeDiv + '"></div>');
  };
  ...
  DTree.prototype.selectVal = function(param) {
    ...
    for (var key in selectInputName) {
      ...
      $("div[dtree-select='" + _this.selectDiv + "']")
        .find("input[dtree-id='" + rootId + "'][name='" + selectInputName[key] + "']")
        .val(param[key] || "");
    }
    ...
    // 返显提示输入框值
    $("div[dtree-select='" + _this.selectDiv + "']")
      .find("input[dtree-id='" + rootId + "'][name='" + _this.selectTipsName + "']")
  ...
  DTree.prototype.useDefaultOrUserDefineFnodeStyle = function(ifont, fnode) {
    ...
    if (typeof fnode === "undefined") {
      ...
      this.usefontStyle.fnode.node.close =
        tempClose == "" ? ifont + " " + this.nodeIconArray[this.fnodeIcon]["close"] : tempClose; // 一级图标中的node节点close图标
      ...
      if (typeof node === "undefined") {
        ...
        if (typeof open === "undefined") {
          this.usefontStyle.fnode.node.open =
            tempOpen == "" ? ifont + " " + this.nodeIconArray[this.fnodeIcon]["open"] : tempOpen; // 一级图标中的node节点open图标
        } else {
  ...
  DTree.prototype.useDefaultOrUserDefineSnodeStyle = function(ifont, snode) {
    ...
    if (typeof snode === "undefined") {
      ...
      if (typeof node === "undefined") {
        this.usefontStyle.snode.node.open = tempOpen == "" ? ifont + " " + this.nodeIconArray[this.nodeIcon]["open"] : tempOpen; // 二级图标中的node节点open图标
        this.usefontStyle.snode.node.close =
          tempClose == "" ? ifont + " " + this.nodeIconArray[this.nodeIcon]["close"] : tempClose; // 二级图标中的node节点close图标
      } else {
        var open = node.open;
        var close = node.close;
        if (typeof open === "undefined") {
  ...
  DTree.prototype.useDefaultOrUserDefineToolbarStyle = function(ifont, toolbar) {
    ...
    if (typeof toolbar === "undefined") {
      this.usefontStyle.toolbar.menubar.movedown =
        tempMovedown == "" ? ifont + " " + this.usefontStyle.menubar.movedown : tempMovedown;
      this.usefontStyle.toolbar.menubar.moveup = tempMoveup == "" ? ifont + " " + this.usefontStyle.menubar.moveup : tempMoveup;
      this.usefontStyle.toolbar.menubar.refresh =
        tempRefresh == "" ? ifont + " " + this.usefontStyle.menubar.refresh : tempRefresh;
      this.usefontStyle.toolbar.menubar.checkAll =
        tempCheckAll == "" ? ifont + " " + this.usefontStyle.menubar.checkAll : tempCheckAll;
      this.usefontStyle.toolbar.menubar.unCheckAll =
        tempUnCheckAll == "" ? ifont + " " + this.usefontStyle.menubar.unCheckAll : tempUnCheckAll;
      this.usefontStyle.toolbar.menubar.invertAll =
        tempInvertAll == "" ? ifont + " " + this.usefontStyle.menubar.invertAll : tempInvertAll;
      ...
      if (typeof menubar === "undefined") {
        ...
        if (typeof moveup === "undefined") {
          this.usefontStyle.toolbar.menubar.moveup =
            tempMoveup == "" ? ifont + " " + this.usefontStyle.menubar.moveup : tempMoveup;
        } else {
        ...
        if (typeof remove === "undefined") {
          this.usefontStyle.toolbar.menubar.remove =
            tempRemove == "" ? ifont + " " + this.usefontStyle.menubar.remove : tempRemove;
        } else {
          this.usefontStyle.toolbar.menubar.remove = ifont + " " + remove;
        }
        if (typeof search === "undefined") {
          this.usefontStyle.toolbar.menubar.search =
            tempSearch == "" ? ifont + " " + this.usefontStyle.menubar.search : tempSearch;
        } else {
  ...
  DTree.prototype.showLineLi = function($li) {
    ...
    if ($ul[0].id == _this.obj[0].id) {
      ...
      if ($pnextLi.length == 0) {
        ...
        if (
          $nextLi.length == 0 &&
          $div.children("cite").attr("data-leaf") == "leaf" &&
          $pnextdiv.children("cite").attr("data-leaf") == "leaf"
        ) {
          $li.removeClass(LI_NAV_FIRST_LINE);
          $li.removeClass(LI_NAV_LINE);
          $li.addClass(LI_NAV_LAST_LINE);
      ...
      for (var i = 0; i < nowListData.length; i++) {
        ...
        $ul.append(
          _this.getLiItemDom(
            ...
            parseData.title(),
            parseData.fmtTitle(),
            parseData.last(childListData.length),
            parseData.ficonClass(),
            parseData.iconClass(),
            parseData.checkArr(),
            level,
            parseData.spread(level),
            parseData.disabled(),
            parseData.hide(),
            parseData.basicData(),
            parseData.recordData(),
            $ul.hasClass(UL_ROOT) ? "root" : "item"
          )
        );
        // 4.有子数据的元素加载子节点
      ...
      for (var i = 0; i < listData.length; i++) {
        ...
        $ul.append(
          _this.getLiItemDom(
            ...
            parseData.title(),
            parseData.fmtTitle(),
            parseData.last(children.length),
            parseData.ficonClass(),
            parseData.iconClass(),
            parseData.checkArr(),
            level,
  ...
  DTree.prototype.parseData = function(data) {
    ...
    return {
      ...
      recordData: function() {
        var recordData = _this.record
          ? event.cloneObj(data, [
              _this.response.treeId,
  ...
  /**
     ...
     * @Param: hide： 是否隐藏
     */
  DTree.prototype.getDom = function(
    treeId,
    parentId,
    title,
    fmtTitle,
    ...
    return {
      ...
      ul: function() {
        //子节点ul
        return last
          ? "<ul class='" + LI_NAV_CHILD + "' data-id='" + treeId + "' dtree-id='" + rootId + "'></ul>"
          : spread
            ? "<ul class='" + LI_NAV_CHILD + " " + NAV_SHOW + "' data-id='" + treeId + "' dtree-id='" + rootId + "'></ul>"
            : "<ul class='" + LI_NAV_CHILD + "' data-id='" + treeId + "' dtree-id='" + rootId + "'></ul>";
      }
    };
  };
  ...
  /**
     ...
     */
  DTree.prototype.getLiItemDom = function(
    treeId,
    parentId,
    title,
    fmtTitle,
    ...
    var li = [
      "<li " +
        ...
        "'" +
        "data-pid='" +
        (isRoot == "root" ? (typeof parentId !== undefined && parentId != "" ? parentId : "-1") : parentId) +
        "'" +
        "dtree-id='" +
        rootId +
        "'" +
  ...
  DTree.prototype.dataInit = function(chooseId) {
    ...
    if ($div && $div.length > 0) {
      ...
      $li_parents
        .children("." + LI_DIV_ITEM)
        .children("i[data-spread]." + event.trimToDot(_this.usefontStyle.fnode.node.close))
        .addClass(_this.usefontStyle.fnode.node.open);
      $li_parents
        .children("." + LI_DIV_ITEM)
        .children("i[data-spread]." + event.trimToDot(_this.usefontStyle.fnode.node.close))
        .removeClass(_this.usefontStyle.fnode.node.close);
      $li_parents
        .children("." + LI_DIV_ITEM)
        .children("i[data-spread]." + event.trimToDot(_this.usefontStyle.snode.node.close))
        .addClass(_this.usefontStyle.snode.node.open);
      $li_parents
        .children("." + LI_DIV_ITEM)
        .children("i[data-spread]." + event.trimToDot(_this.usefontStyle.snode.node.close))
        .removeClass(_this.usefontStyle.snode.node.close);
    }
  ...
  DTree.prototype.navThis = function(id) {
    ...
    var $div =
      typeof id === "object"
        ? id
        : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']").length == 0
          ? null
          : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']");
    if ($div != null) {
      _this.cancelNavThis();
      $div.addClass(NAV_THIS);
      $div.addClass(_this.style.itemThis);
  ...
  DTree.prototype.partialRefreshEdit = function($div, data) {
    var _this = this;
    ($ul = $div.next("ul")), ($p_li = $div.parent("li"));
    ...
    if (data) {
      if (typeof data === "object") {
        ...
        if (parseData.treeId()) {
          var replaceDom = _this.replaceDom(
            $div,
            parseData.treeId(),
            parseData.last(0),
            parseData.spread(),
  ...
  DTree.prototype.chooseDataInit = function(chooseIds) {
    ...
    if (chooseId.length) {
      ...
      for (var i = 0; i < chooseIdArrs.length; i++) {
        ...
        for (var j = 0; j < chooseIdArr.length; j++) {
          var $i = _this.obj.find(
            "i[dtree-click='" + eventName.checkNodeClick + "'][data-type=" + i + "][data-id=" + chooseIdArr[j] + "]"
          );
    ...
    // 展开选中节点的父节点
    var $li_parents = _this.obj
      .find("i[dtree-click='" + eventName.checkNodeClick + "'][data-checked='1']")
      .parents("." + LI_NAV_ITEM);
    $li_parents.children("ul").addClass(NAV_SHOW);
    $li_parents
      .children("." + LI_DIV_ITEM)
  ...
  DTree.prototype.checkAllOrNot = function($i) {
    ...
    if ($i.attr("data-checked") == "1") {
      ...
      for (var i = 1, item = $parent_li; i < item.length; i++) {
        var flag = item
          .eq(i)
          .find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "'][data-checked='1']").length;
        if (flag == 0) {
          //把父级去掉选中
  ...
  DTree.prototype.setAndGetCheckbarNodesParam = function(requestParamFlag) {
    ...
    } else if (_this.checkbarData == "all") {
      //记录全部数据
    ...
    } else if (_this.checkbarData == "choose") {
      //记录选中数据
    ...
    } else if (_this.checkbarData == "halfChoose") {
      //记录选中和半选数据
      _this.obj.find("i[data-par][data-checked='1'][dtree-disabled='false']").each(function() {
        var $i = $(this),
          $div = $i.closest("." + LI_DIV_ITEM);
  ...
  DTree.prototype.getCheckbarNodeParam = function($div, $i) {
    ...
    temp_node.context =
      typeof _this.formatter.title === "function"
        ? _this.getNodeDom($div).cite().attr("data-title")
        : _this.getNodeDom($div).cite().text();
  ...
  DTree.prototype.initTreePlus = function() {
    ...
    _this.toolbarMenu = {};
    if (_this.menubar && _this.menubarTips.group && _this.menubarTips.group.length > 0)
      _this.obj.before(
        "<div class='" + LI_DIV_MENUBAR + "' id='dtree_menubar_" + _this.obj[0].id + "'><div class='layui-btn-group'></div></div>"
      );
    if (_this.toolbar) {
      if (_this.toolbarWay == "contextmenu") {
        ...
        if (_this.toolbarShow && _this.toolbarShow.length > 0) {
          _this.obj.before(
            "<div class='" +
              LI_DIV_TOOLBAR +
              " layui-nav' id='dtree_toolbar_" +
              _this.obj[0].id +
              "'><div class='layui-nav-item'><dl class='layui-nav-child layui-anim'></dl></div></div>"
          );
        }
      }
    }
  ...
  DTree.prototype.getMenubarDom = function(menu) {
    ...
    switch (menu) {
      case defaultMenu.moveDown:
        gg =
          "<button type='button' class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='" +
          rootId +
          "' d-menu='" +
          defaultMenu.moveDown +
          "' title='展开全部节点'><i class='" +
          _this.usefontStyle.menubar.movedown +
          "'></i></button>";
        break;
      case defaultMenu.moveUp:
        gg =
          ...
          "' d-menu='" +
          defaultMenu.moveUp +
          "' title='收缩全部节点'><i class='" +
          _this.usefontStyle.menubar.moveup +
          "'></i></button>";
        break;
      case defaultMenu.refresh:
        gg =
          ...
          defaultMenu.refresh +
          "' title='刷新'><i class='" +
          _this.usefontStyle.menubar.refresh +
          "'></i></button>";
        break;
      case defaultMenu.checkAll:
        gg =
          _this.checkbar && _this.checkbarType != "only"
            ? "<button type='button' class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='" +
              rootId +
              "' d-menu='" +
              defaultMenu.checkAll +
              "' title='全选节点'><i class='" +
              _this.usefontStyle.menubar.checkAll +
              "'></i></button>"
            : "";
        break;
        ...
        gg =
          _this.checkbar && _this.checkbarType != "only"
            ? "<button type='button' class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='" +
              ...
              defaultMenu.unCheckAll +
              "' title='全不选节点'><i class='" +
              _this.usefontStyle.menubar.unCheckAll +
              "'></i></button>"
            : "";
        break;
        ...
        gg =
          _this.checkbar && _this.checkbarType != "only"
            ? "<button type='button' class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='" +
              ...
              defaultMenu.invertAll +
              "' title='反选节点'><i class='" +
              _this.usefontStyle.menubar.invertAll +
              "'></i></button>"
            : "";
        break;
      case defaultMenu.remove:
        gg = _this.checkbar
          ? "<button type='button' class='layui-btn layui-btn-sm layui-btn-primary' dtree-id='" +
            ...
            "' d-menu='" +
            defaultMenu.remove +
            "' title='删除选中节点'><i class='" +
            _this.usefontStyle.menubar.remove +
            "'></i></button>"
          : "";
        break;
      case defaultMenu.searchNode:
        gg =
          ...
          defaultMenu.searchNode +
          "' title='查询节点'><i class='" +
          _this.usefontStyle.menubar.search +
          "'></i></button>";
        break;
    }
    return gg;
  ...
  DTree.prototype.getMenubarToolDom = function(menu) {
    ...
    switch (menu) {
      case defaultMenu.moveDown:
        _this.toolbarMenu[defaultMenu.moveDown] = _this
          .setToolbarDom()
          .setMenuToolbarOption(
            defaultMenu.moveDown,
            _this.toolbarStyle.title,
            _this.usefontStyle.toolbar.menubar.movedown,
            "展开全部"
          );
        break;
      case defaultMenu.moveUp:
        _this.toolbarMenu[defaultMenu.moveUp] = _this
          .setToolbarDom()
          .setMenuToolbarOption(defaultMenu.moveUp, _this.toolbarStyle.title, _this.usefontStyle.toolbar.menubar.moveup, "收缩全部");
        break;
      case defaultMenu.refresh:
        _this.toolbarMenu[defaultMenu.refresh] = _this
          .setToolbarDom()
          .setMenuToolbarOption(defaultMenu.refresh, _this.toolbarStyle.title, _this.usefontStyle.toolbar.menubar.refresh, "刷新");
        break;
      case defaultMenu.checkAll:
        if (_this.checkbar && _this.checkbarType != "only")
          _this.toolbarMenu[defaultMenu.checkAll] = _this
            .setToolbarDom()
            .setMenuToolbarOption(
              defaultMenu.checkAll,
              _this.toolbarStyle.title,
              _this.usefontStyle.toolbar.menubar.checkAll,
              "全选节点"
            );
        break;
      case defaultMenu.unCheckAll:
        if (_this.checkbar && _this.checkbarType != "only")
          _this.toolbarMenu[defaultMenu.unCheckAll] = _this
            .setToolbarDom()
            .setMenuToolbarOption(
              defaultMenu.unCheckAll,
              _this.toolbarStyle.title,
              _this.usefontStyle.toolbar.menubar.unCheckAll,
              "全不选节点"
            );
        break;
      case defaultMenu.invertAll:
        if (_this.checkbar && _this.checkbarType != "only")
          _this.toolbarMenu[defaultMenu.invertAll] = _this
            .setToolbarDom()
            .setMenuToolbarOption(
              defaultMenu.invertAll,
              _this.toolbarStyle.title,
              _this.usefontStyle.toolbar.menubar.invertAll,
              "反选节点"
            );
        break;
      case defaultMenu.remove:
        if (_this.checkbar)
          _this.toolbarMenu[defaultMenu.remove] = _this
            .setToolbarDom()
            .setMenuToolbarOption(
              defaultMenu.remove,
              _this.toolbarStyle.title,
              _this.usefontStyle.toolbar.menubar.remove,
              "删除选中"
            );
        break;
      case defaultMenu.searchNode:
        _this.toolbarMenu[defaultMenu.searchNode] = _this
          .setToolbarDom()
          .setMenuToolbarOption(
            defaultMenu.searchNode,
            _this.toolbarStyle.title,
            _this.usefontStyle.toolbar.menubar.searchNode,
            "查询"
          );
        break;
    }
  ...
  DTree.prototype.getExtMenubarToolDom = function(menu) {
    var _this = this;
    _this.toolbarMenu[menu.menubarId] = _this
      .setToolbarDom()
      .setMenuToolbarOption(menu.menubarId, menu.title, _this.usefontStyle.toolbar.menubarExt + " " + menu.icon, "");
  };
  ...
  DTree.prototype.getToolbarDom = function() {
    ...
    if (toolbarShow.length > 0) {
      for (var i = 0; i < toolbarShow.length; i++) {
        ...
        if (show == "pulldown") {
          _this.toolbarMenu[defaultTool.pulldown] = _this
            .setToolbarDom()
            .setToolbarOption(defaultTool.pulldown, _this.toolbarStyle.title, _this.usefontStyle.toolbar.pulldown, "展开");
        }
        if (show == "pullup") {
          _this.toolbarMenu[defaultTool.pullup] = _this
            .setToolbarDom()
            .setToolbarOption(defaultTool.pullup, _this.toolbarStyle.title, _this.usefontStyle.toolbar.pullup, "收缩");
        }
        if (show == "add") {
          _this.toolbarMenu[defaultTool.addTool] = _this
            .setToolbarDom()
            .setToolbarOption(defaultTool.addTool, _this.toolbarStyle.title, _this.usefontStyle.toolbar.add, "新增");
        }
        if (show == "edit") {
          _this.toolbarMenu[defaultTool.editTool] = _this
            .setToolbarDom()
            .setToolbarOption(defaultTool.editTool, _this.toolbarStyle.title, _this.usefontStyle.toolbar.edit, "编辑");
        }
        if (show == "delete") {
          _this.toolbarMenu[defaultTool.delTool] = _this
            .setToolbarDom()
            .setToolbarOption(defaultTool.delTool, _this.toolbarStyle.title, _this.usefontStyle.toolbar.del, "删除");
        }
      }
    }
    if (toolbarExt.length > 0) {
      for (var i = 0; i < toolbarExt.length; i++) {
        ...
        _this.toolbarMenu[ext.toolbarId] = _this
          .setToolbarDom()
          .setToolbarOption(ext.toolbarId, ext.title, _this.usefontStyle.toolbarExt + " " + ext.icon, "");
      }
    }
  };
        ...
        if (toolbarWay == "contextmenu") {
          if (toolbarMenu) {
            ...
            for (var key in toolbarMenu) {
              _this.obj
                .prevAll("div#dtree_toolbar_" + _this.obj[0].id)
                .find("div.layui-nav-item>dl.layui-nav-child")
                .append(toolbarMenu[key]);
            }
          }
        } else if (toolbarWay == "fixed" || toolbarWay == "follow") {
  ...
  DTree.prototype.toolbarListener = function(tool, $div) {
    ...
    switch (tool) {
      ...
      case defaultTool.delTool:
        layer.confirm("确定要删除该" + _this.toolbarStyle.title + "？", { icon: 3, title: "删除" + _this.toolbarStyle.title }, function(
          index
        ) {
  ...
  DTree.prototype.loadToolBar = function(title, name) {
    ...
    switch (name) {
      case defaultTool.addTool:
        var addNodeBarDef = [
          { label: "当前选中", name: "nodeTitle", type: "text", value: title, defElem: "nowChoose", readonly: true },
          {
        ...
        var addNodeBar = [
          '<div class="' +
            TOOLBAR_TOOL +
            '"><form class="layui-form layui-form-pane" lay-filter="dtree_addNode_' +
            _this.obj[0].id +
            '_form">'
        ];
      ...
      case defaultTool.editTool:
        ...
        var editNodeBar = [
          '<div class="' +
            TOOLBAR_TOOL +
            '"><form class="layui-form layui-form-pane" lay-filter="dtree_editNode_' +
            _this.obj[0].id +
            '_form">'
        ];
        //2. 用户自定义的节点内容
    ...
    return {
      text: function() {
        return [
          ...
          '<input type="text" class="layui-input f-input" value="' +
            val +
            '" placeholder="' +
            placeholder +
            '" lay-verify="' +
      ...
      textarea: function() {
        return [
          ...
          '<label class="layui-form-label" title="' + nodeBarContents.label + '">' + nodeBarContents.label + "：</label>",
          '<div class="layui-input-block f-input-par">',
          '<textarea class="layui-textarea f-input" value="' +
            val +
            '" placeholder="' +
            placeholder +
            '" lay-verify="' +
      ...
      hidden: function() {
        return [
          '<input type="hidden" class="layui-input f-input" value="' + val + '" lay-verify="' + verify + '" ',
          id != "" ? 'id="' + id + '" ' : "",
          name != "" ? 'name="' + name + '" ' : "",
          readonly ? "readonly " : "",
          disabled ? "disabled " : "",
      ...
      select: function() {
        var optionsData =
          typeof nodeBarContents.optionsData === "object" ? nodeBarContents.optionsData : nodeBarContents.optionsData();
        var options = "";
      ...
      submit: function() {
        ...
        return [
          '<button type="button" class="layui-btn layui-btn-normal btn-w100" lay-submit lay-filter="' + filter + '" ',
          id != "" ? 'id="' + id + '" ' : "",
          name != "" ? 'name="' + name + '" ' : "",
          ">" + val + "</button>"
        ].join("");
      ...
      button: function() {
        return [
          '<button type="button" class="layui-btn layui-btn-normal btn-w100" ',
          id != "" ? 'id="' + id + '" ' : "",
          name != "" ? 'name="' + name + '" ' : "",
          " >" + val + "</button>"
        ].join("");
      ...
      reset: function() {
        return [
          '<button type="reset" class="layui-btn layui-btn-primary btn-w100" ',
          id != "" ? 'id="' + id + '" ' : "",
          name != "" ? 'name="' + name + '" ' : "",
          ">" + val + "</button>"
        ].join("");
  ...
  DTree.prototype.getNodeDom = function(id) {
    ...
    return {
      ...
      nowDiv: function() {
        // 获取当前选中节点，没有则返回null
        return _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id]").parent().find("." + NAV_THIS)
          .length == 0
          ? null
          : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id]").parent().find("." + NAV_THIS);
      },
      nowOrRootDiv: function() {
        // 获取当前选中节点，没有则返回根节点下的第一个div
        return _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id]").parent().find("." + NAV_THIS)
          .length == 0
          ? _this.obj.children("li").eq(0).children("div").eq(0)
          : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id]").parent().find("." + NAV_THIS);
      },
      nowOrRootUl: function() {
        ...
        return _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id]").parent().find("." + NAV_THIS)
          ...
          ? _this.obj
          : _this.obj
              .find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id]")
              .parent()
              .find("." + NAV_THIS)
              .next("ul");
      }
  ...
  DTree.prototype.bindBrowserEvent = function() {
    ...
    if (_this.toolbar) {
      if (_this.toolbarWay == "contextmenu") {
        ...
        _this.obj.on(
          "contextmenu",
          "div[dtree-click='" + eventName.itemNodeClick + "'][d-contextmenu='true'][dtree-disabled='false']",
          function(e) {
            var $div = $(this),
              node = _this.getNodeParam($div);
            ...
            _this
              .setToolbarDom()
              .setToolbarPlace(
                _this.toolbarFun.loadToolbarBefore(event.cloneObj(_this.toolbarMenu), _this.getRequestParam(node), $div)
              );
        ...
        _this.obj.on("mouseover mouseout", "div[dtree-click='" + eventName.itemNodeClick + "'][dtree-disabled='false']", function(
          event
        ) {
          var $div = $(this),
  ...
  DTree.prototype.unbindBrowserEvent = function() {
    ...
    if (_this.select) {
      ...
      $("div[dtree-id='" + _this.obj[0].id + "'][dtree-card='" + _this.selectCardDiv + "']").removeClass(
        "dtree-select-show layui-anim layui-anim-upbit"
      );
  ...
  var dtree = {
    ...
    getParentParam: function(dTree, id) {
      // 获取参数的上级节点
      if (typeof dTree === "string") {
        dTree = DTrees[dTree];
      }
    ...
    getAllParentParam: function(dTree, id) {
      // 获取参数的全部上级节点
      if (typeof dTree === "string") {
        dTree = DTrees[dTree];
      }
    ...
    getChildParam: function(dTree, id) {
      // 获取参数的全部下级节点
      if (typeof dTree === "string") {
        dTree = DTrees[dTree];
      }
    ...
    getCheckbarJsonArrParam: function(dTree) {
      // 获取复选框选中值
      if (typeof dTree === "string") {
        dTree = DTrees[dTree];
      }
    ...
    dataInit: function(dTree, chooseId) {
      // 初始化选中树，针对数据反选
      if (typeof dTree === "string") {
        dTree = DTrees[dTree];
      }
    ...
    chooseDataInit: function(dTree, chooseIds) {
      // 初始化复选框选中，针对数据反选
      if (typeof dTree === "string") {
        dTree = DTrees[dTree];
      }
    ...
    changeCheckbarNodes: function(dTree) {
      //判断复选框是否发生变更
      if (typeof dTree === "string") {
        dTree = DTrees[dTree];
      }
    ...
    selectVal: function(dTree, param) {
      // select模式设置输入框选中值
      if (typeof dTree === "string") {
        dTree = DTrees[dTree];
      }
