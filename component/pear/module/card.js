layui.define(["table", "laypage", "jquery", "element"], function(exports) {
  ...
  card.prototype.init = function() {
    ...
    if (!!url) {
      ...
      if (option.page) {
        ...
        for (
          var i = (option.currentPage - 1) * option.limit;
          i < option.currentPage * option.limit && i < option.alldata.length;
          i++
        ) {
  ...
  function createCard(elem, linenum, item, no) {
    ...
    var card =
      "<div id=" +
      item.id +
      ' onclick="cardTableCheckedCard(' +
      elem +
      ',this)" class="layui-col-md' +
      line +
      ' ew-datagrid-item" data-index="' +
      no +
      '" data-number="1"> <div class="project-list-item"> <div class="project-list-item-cover" style="background-image: url(' +
      item.image +
      ');"></div> <div class="project-list-item-body"> <h2 class="layui-elip">' +
      item.title +
      '</h2> <div class="project-list-item-text layui-text">' +
      item.remark +
      '</div> <div class="project-list-item-desc"> <span class="time">' +
      item.time +
      '</span> <div class="ew-head-list"></div> </div> </div > </div > </div > ';
    return card;
  }
