layui.define(["jquery", "element"], function(exports) {
  ...
  pearTab.prototype.render = function(opt) {
    ...
    $("body .layui-tab[lay-filter='" + option.elem + "'] .layui-tab-title").on("contextmenu", "li", function(e) {
      ...
      var menu =
        "<ul><li class='item' id='" +
        option.elem +
        "closeThis'>关闭当前</li><li class='item' id='" +
        option.elem +
        "closeOther'>关闭其他</li><li class='item' id='" +
        option.elem +
        "closeAll'>关闭所有</li></ul>";
  ...
  pearTab.prototype.addTab = function(opt) {
    ...
    if (opt.close) {
      title +=
        '<span class="pear-tab-active"></span><span class="able-close title">' +
        opt.title +
        '</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
    } else {
      title +=
        '<span class="pear-tab-active"></span><span class="disable-close title">' +
        opt.title +
        '</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
    }
    element.tabAdd(this.option.elem, {
      ...
      content:
        ...
        opt.url +
        '" style="width:100%;height:100%;" allowfullscreen="true"></>',
      id: opt.id
    });
    tabData.push(opt);
  ...
  function tabIframeLoading(elem, id) {
    var load =
      '<div id="pear-tab-loading' +
      index +
      '" class="pear-tab-loading">' +
  ...
  function createTab(option) {
    ...
    $.each(option.data, function(i, item) {
      ...
      if (option.index == index) {
        // 处 理 显 示 内 容
        content +=
          '<div class="layui-show layui-tab-item"><iframe id="' +
          item.id +
          '" data-frameid="' +
          item.id +
          '"  src="' +
        ...
        // 处 理 显 示 内 容
        content +=
          '<div class="layui-tab-item"><iframe id="' +
          item.id +
          '" data-frameid="' +
          item.id +
          '"  src="' +
  ...
  function mousewheelAndTouchmoveHandler(option) {
    ...
    $bodyTab.on("mousewheel DOMMouseScroll", function(e) {
      ...
      var delta =
        (e.originalEvent.wheelDelta && (e.originalEvent.wheelDelta > 0 ? "top" : "down")) || // chrome & ie
