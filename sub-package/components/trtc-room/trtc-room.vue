				...
				<view data-type="template" data-is="1v1" data-attr="pusher, streamList, debug">
					<view class="template-1v1">
						...
						<view
							...
							:key="streamID"
							v-if="config.isOnlyDisplayMe ? false : item.src && (item.hasVideo || item.hasAudio)"
						...
						<!-- 显示自己的画面 start 层级非常高 -->
						<view :class="'view-container pusher-container ' + (pusher.isVisible ? '' : 'none') + ' ' + (JSON.stringify(streamList) == '[]' ? 'fullscreen' : (config.isOnlyDisplayMe ? 'fullscreen' : ''))">
							<live-pusher
								class="pusher"
								:url="pusher.url"
								:mode="pusher.mode"
								...
								:waiting-image="pusher.waitingImage || 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/loading.png'"
								:debug="debug"
							...
							></live-pusher>
							<cover-view class="loading" v-if="streamList.length === 0 && !isOtherExitRoom">
								<cover-view class="loading-img"><cover-image src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/loading.png" class="rotate-img"></cover-image></cover-view>
								<cover-view class="loading-text">等待接听中...</cover-view>
							</cover-view> 
							<cover-view class="loading1" v-if="streamList.length === 0 && isOtherExitRoom">
								<cover-view class="loading-img1">
									<cover-image class="rotate-img1" src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/tx.png"></cover-image>
								</cover-view>
								<cover-view class="loading-text">通话已结束</cover-view>
							</cover-view>
						</view>
						<!-- 显示自己的画面 end 层级非常高 -->
						
						<cover-view class="handle-btns">
							<cover-view class="btn-normal" @tap="toggleAudioFun">
								<cover-image style="width: 60%;height: 60%;"
									:src="pusher.enableMic ? 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/audio-true.png' : 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/audio-false.png'"
								></cover-image>
							</cover-view>
							<cover-view class="btn-normal" @tap="switchCamera">
								<cover-image style="width: 60%;height: 60%;" src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/switch.png"></cover-image>
							</cover-view>
							<!-- <view class="btn-normal" bindtap="toggleVideoFun">
        <image src="{{pusher.enableCamera? 'https://file.yuanzhoulvwego.com/prod/uploadFiles/components/trtc-room/static/camera-true.png': 'https://file.yuanzhoulvwego.com/prod/uploadFiles/components/trtc-room/static/camera-false.png'}} "></image>
      </view> -->
							<cover-view class="btn-normal" @tap="toggleSoundModeFun">
								<cover-image style="width: 60%;height: 60%;"
									:src="
										streamList[0].soundMode === 'ear'
											? 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/phone.png'
											: 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/speaker-true.png'
									"
								></cover-image>
							</cover-view>
						</cover-view>
						<cover-view class="bottom-btns">
							<cover-view class="btn-hangup" @tap="hangUpFun">
								<cover-image style="width: 60%;height: 60%;" src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/hangup.png"></cover-image>
							</cover-view>
							<!-- <view class="btn-normal" @tap="remoteDoorOpeningFun"><image src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/door.png"></image></view> -->
						</cover-view>
					</view>
				</view>
			</view>
				...
				<view data-type="template" data-is="grid" data-attr="pusher, streamList, debug, panelName">
					<view :class="'template-grid ' + (streamList.length < 6 ? 'stream-' + streamList.length : 'stream-5')">
							...
							<view class="operation-bar">
								<view class="btn-normal" @tap="handleSubscribeRemoteAudioFun" :data-user-i-d="item.userID" :data-stream-type="item.streamType">
									<image
										:src="
											item.muteAudio
												? 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/speaker-false.png'
												: 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/speaker-true.png'
										"
									></image>
								</view>
								<view class="btn-normal" @tap="handleSubscribeRemoteVideoFun" :data-user-i-d="item.userID" :data-stream-type="item.streamType">
									<image
										:src="
											item.muteVideo
												? 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/camera-false.png'
												: 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/camera-true.png'
										"
									></image>
								</view>
								<view class="btn-normal" @tap="toggleFullscreenFun" :data-user-i-d="item.userID" :data-stream-type="item.streamType">
									<image src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/fullscreen.png"></image>
								</view>
							</view>
						...
						<view :class="'view-container pusher-container ' + (pusher.isVisible ? '' : 'none')">
							...
							></live-pusher>
							<view class="operation-bar">
								<view class="btn-normal" @tap="switchMemberListPanelFun"><image src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/list.png"></image></view>
								<view class="btn-normal" @tap="switchSettingPanelFun"><image src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/setting.png"></image></view>
								<view class="btn-normal btn-hangup" @tap="hangUpFun"><image src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/hangup.png"></image></view>
							</view>
						</view>
							...
							<view class="panel-body">
								<scroll-view class="scroll-container" scroll-y="true">
									...
									<view class="setting-option">
										...
										<view class="btn-normal" @tap="toggleAudioFun">
											<image
												:src="
													pusher.enableMic
														? 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/audio-true.png'
														: 'https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/audio-false.png'
												"
											></image>
										</view>
									</view>
									<view class="setting-option">
										...
										<view class="btn-normal" @tap="switchCamera"><image src="https://file.yuanzhoulvwego.com/prod/uploadFiles/img/components/trtc-room/static/switch.png"></image></view>
									</view>
									<view class="setting-option">
									...
									<view class="setting-option">
										...
										<switch color="#0081ff" :checked="pusher.enableAgc" data-key="enableAgc" data-value="true" @change="setPuserPropertyFun"></switch>
									</view>
									<view class="setting-option">
										<view class="label">开启ANS</view>
										<switch color="#0081ff" :checked="pusher.enableAns" data-key="enableAns" data-value="true" @change="setPuserPropertyFun"></switch>
									</view>
									<view class="setting-option">
...
let timer = null;   // 定义一个定时器，时间为30s，30s后如果远端用户没有接听，则当前退出视频通话界面（退出房间）
export default {
	data() {
		return {
			pusher: null,
	...
	props: {
		...
		config: {
			...
			default: () => ({
				...
				devSn: '',
				relayIndex: 1,
				isOnlyDisplayMe: false    // 默认显示自己以及对方的画面
			})
		}
	},
	watch: {
	...
	methods: {
		...
		enterRoom: function(params) {
			...
			timer = setTimeout(() => {
				// 1v1下，远端用户在30s内没有接听，将自动退出视频通话界面
				if (this.streamList.length === 0) {
					this.exitRoom();
					this.isOtherExitRoom = true;
					setTimeout(() =>{
						uni.navigateBack({
							delta: 1
						});
		...
		bindEventFun: function() {
			...
			this.userController.on(EVENT.REMOTE_USER_LEAVE, event => {
				...
				// 远端用户（对方）退出房间（退出通话界面），如果streamList为空数组，那么自己也退出通话界面
				if (this.streamList.length == 0) {
					// this.hangUpFun();
					if (timer) {
						clearTimeout(timer);
		 ...
		 * 远程开门
		 */
		remoteDoorOpeningFun: function() {
			let config = this.config;
			this.$showToast("relayIndex: " + config.relayIndex + ", devSn: " + config.devSn);
			this.$req.request({
				hasCallback: true,
				timeout: 3000,
				...
				success: res => {
					this.$showToast("res: " + JSON.stringify(res));
					if (res.code === 0) {
						// 开门成功
						setTimeout(() => {
							this.$showToast("开门成功");
							this.$sdkUtil.openDoorAduioPlay();
						}, 1000);
					} else {
						// this.$showToast("开门失败");
					}
