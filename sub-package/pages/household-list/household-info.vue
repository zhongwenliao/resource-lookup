    ...
    <view class="info-list">
      <view class="info-list-with-img">
        <view class="image-view">
          <image
            class="uni-list-cell-img"
            mode="aspectFit"
            :src="
              getInfo.faceImage !== undefined
                ? getInfo.faceImage
                : 'http://www.thinmoo.com/wp-content/uploads/2020/10/static-img.png'
            "
          >
          </image>
        </view>
      ...
      <view v-if="isAppAddMemberCanAddCard" class="info-list-cell border-bottom">
        ...
        <ul style="display: flex; flex-wrap: wrap" v-if="getInfo.cardNos">
          <li
            v-for="(item, i) in getInfo.cardNos.split(',')"
            @click="delcardNo(item, i)"
      ...
      <view v-if="getInfo.isBindDoorLock" class="info-list-cell border-bottom">
        ...
        <ul style="display: flex; flex-wrap: wrap">
          <li
            v-for="(item, i) in getInfo.persBioFingerprintList"
            @click="deleFingerprintBtn(item, i)"
            style="display: flex; align-items: center; padding: 10upx"
          >
            <view
              style="
      ...
      <view class="info-list-cell border-bottom" v-if="getInfo.identityType !== 1 && tenant != 1">
        ...
        <view style="float: right; color: #00aa7f" @click="editRentDate">{{ house["edit-rental"] || "续租" }} </view>
      </view>
      <view class="dividing-line"></view>
      ...
      <view
        v-if="getInfo.isBindDoorLock && getInfo.sqCommunityParam.fingerprintSyncMode === 1"
        style="background: #f5f5f5"
      >
        <view class="info-list-cell">
          <button class="green-bg" @click="addFingerprintBtn">{{ house["add_fingerprint"] || "添加指纹" }}</button>
        </view>
      </view>
    </view>
  ...
  methods: {
    ...
    addFingerprintBtn() {
      ...
      uni.showModal({
        ...
        content:
          that.house["Input_fingerprint_prompt"] ||
          "请根据设备提示，蓝灯亮起按下手指，绿灯亮起抬起手指，连续按压三次即可录入成功。",
        success: function (res) {
          if (res.confirm) {
    ...
    clickWriteCard(index) {
      ...
      if (index == 0) {
        ...
        uni.navigateTo({
          ...
          url: "/sub-package/pages/household-list/door-opening?memberObj=" + JSON.stringify(this.memberObj),
        })
      }
    },
        ...
        success(res) {
          if (res.confirm) {
            ...
            if (_this.devInfo.isSupportNetwork == 0) {
              ...
              if (
                _this.getInfo &&
                _this.getInfo.sqCommunityParam &&
                _this.getInfo.sqCommunityParam.fingerprintSyncMode == 1
              ) {
                _this.deleteMember()
                return
              }
              ...
              setTimeout(() => {
                _this.$sdkUtil.deleteFingerprints(_this.devInfo, fingerprintsIds, (res) => {
                  if (res.errorCode === 0) {
                    _this.deleteMember()
                  } else {
                    console.log(res)
