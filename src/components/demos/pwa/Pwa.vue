  ...
  methods: {
    registerSW () {
      ...
      navigator.serviceWorker.register('/static/sw.js', { scope: '/' })
        .then(onOk)
        .catch(() => {
          // 降级：服务器未返回 Service-Worker-Allowed 头时根作用域会被拒绝，
          // 默认作用域（/static/）可注册成功，但无法拦截页面请求
          this.swState = '根作用域被拒（缺 Service-Worker-Allowed 头），降级为默认作用域…';
          return navigator.serviceWorker.register('/static/sw.js').then(onOk);
        })
        .catch(onFail);
    },
    trackState (reg) {
    ...
    testPolicy (file, policy) {
      ...
      fetch('/static/pwa-demo/' + file + '?t=' + Date.now())
