...
export default {
  ...
  methods: {
    ...
    loadTile (z, x, y) {
      const key = z + '/' + x + '/' + y;
      const t0 = performance.now();
      this.$set(this.tiles, key, { z, x, y, status: 'loading', url: '', source: '', cost: 0, updated: false });
      this.stats.requests++;
      ...
      const done = (blob, source, cost) => {
        ...
        if (source === 'error') {
          this.$set(this.tiles, key, { z, x, y, status: 'error', url: '', source: 'error', cost, updated: false });
          return;
        }
        const prev = this.tiles[key];
        if (prev && prev.url) URL.revokeObjectURL(prev.url);
        this.$set(this.tiles, key, { z, x, y, status: 'ok', url: URL.createObjectURL(blob), source, cost, updated: false });
      };
          ...
          if (hit) {
            ...
            if (this.strategy === 'swr') {
              this.fetchFromNetwork(z, x, y, cache, (blob, source, cost) => {
                ...
                this.$set(this.tiles, key, { z, x, y, status: 'ok', url: URL.createObjectURL(blob), source: 'cache', cost, updated: true });
              });
            }
            return;
          }
    ...
    runBench () {
      ...
      const coords = [];
      for (let i = 0; i < 8; i++) {
        coords.push({ z, x: bx + (i % 4), y: by + Math.floor(i / 4) });
      }
      this.benching = true;
