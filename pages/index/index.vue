<template>
	<view class="content">
		...
		<view class="bg-box">
			<view class="bg">
				<u-navbar :is-back="false" :immersive="false" back-icon-color="#fff" title-color="#fff"
					...
					<view class="navbar-content u-m-l-38">
						<view @click="selectZone" class="u-flex u-font-lg2 u-text-color-fff">
							<u-icon class="u-m-r-18" color="#fff" name="https://file.yuanzhoulvwego.com/prod/uploadFiles/dingwei.png" size="30">
							</u-icon>
							<text>
					...
					<view slot="right" @click="ScanCode">
						<u-icon color="#fff" name="scan" size="44">
						</u-icon>
					</view>
				</u-navbar>
			</view>
			<view style="margin-top: -240upx;">
				<view class="userImage">
					<u-swiper bg-color="linear-gradient(#709BFB, #7CA3FB)" border-radius="20" height="360"
						:list="bannerList" :name="'imagePath'">
					</u-swiper>
				</view>
			</view>
		</view>
			...
			<u-card :foot-border-top="false" :head-border-bottom="false" :border="true">
				...
				<u-cell-group class="content-notice" :border="false" slot="body">
					...
					<block v-if="noticeList.length>0">
						<u-cell-item :title-style="{fontSize:'32upx'}" v-for="(item,index) in noticeList" :key="index"
							@click="goNoticeList(item.id)" :border-bottom="false" :title="item.title" value="">
							<view slot="icon" :style="{backgroundColor: noticeColor(index)}" class="yuan">
							</view>
						</u-cell-item>
					</block>
					<view v-else class="u-text-center u-p-b-30" style="">
						暂无通知信息
					</view>
			...
			<u-card :foot-border-top="false" :head-border-bottom="false" :border="true">
				<u-cell-group class="head_group" :border="false" slot="head">
					<u-cell-item @click="goPage({url: '/sub-package2/pages/home/merchants/index'})" hover-class="none"
						...
						<u-icon margin-left="10" label="  " size="30" slot="icon" name="https://file.yuanzhoulvwego.com/prod/uploadFiles/zhaoshangxinxi.png">
						</u-icon>
					</u-cell-item>
				...
				<view class="u-p-10 u-p-b-30" slot="body">
					<block v-if="merchantsList.length>0">
						<view v-for="(item,index) in merchantsList" :key="index" @click="goMerchantsDetails(item)">
							...
							<u-gap v-if="merchantsList.length != (index+1)" height="1"
								class="u-m-l-20 u-m-r-20 u-m-t-30 u-m-b-30" bg-color="#F1F2F3"></u-gap>
						</view>
					</block>

					<view v-else class="u-text-center" style="">
						暂无招商信息
					</view>
				</view>
			</u-card>
		...
		onPageScroll(e) {
			...
			if (this.pageScrollTop > 0) {
				this.backgroundImage =
					'linear-gradient(-25deg, #e0e6fd 0%, #ffffff 0%, #7EA4FB  0%, #709BFB 100%) !important'
			} else {
				this.backgroundImage = ' '
			}
		},
			...
			cidAddOrUpdateCid() {
				...
				uni.getPushClientId({
					success: (res) => {
						...
						this.$api('user.addOrUpdateCid', param).then(res => {
							console.log(res);
							if (res.code == 200) {
								console.log('cid')
							}
