	...
	<view class="content" :class="tableData.length < 8 ? 'contentData' : ''">
		<block v-for="(item, index) in tableData" :key="index">
			<view class="hens_list">
				...
				<view class="hens_view">
					<text>当前余额(元)：{{ item.moneyRemain }}</text>
	...
	methods: {
		showinfo(page){
			...
			this.$http("userCapitalListUser",JSON.stringify({
				...
				uni.hideLoading();
				console.log(res.data.capitalList);
				
				if(res.data.code==0){
					this.tableData = res.data.capitalList.datas;
					this.total = res.data.capitalList.pgInfo.total_num
					this.loading = false;
