			...
			<view slot="top" class="tops">
				<view class="top-top" :style="topBackground">
					...
					<view class="top-profile-photo">
						<u-image width="143upx" height="143upx" shape="circle"
							:src="corporateLogo ? corporateLogo : bannerImage">
					...
					<view v-for="(item, index) in dataList" :key="index">
						<u-cell-item :arrow="true" @click="toStaffList(item)">
							<u-image slot="icon" width="88upx" height="88upx" shape="circle"
								:src="item.avaer?item.avaer:'../../../static/home/employeeManagement/bumen.png'">
								<u-loading slot="loading"></u-loading>
								<view slot="error" style="font-size: 24upx;">加载失败</view>
							</u-image>
							<text slot="title"
								class="content-title">{{ item.roleName + ' ' + ` (${item.count})` }}</text>
						</u-cell-item>
					</view>
				</u-cell-group>
			</view>
	...
	export default {
		...
		methods: {
			getContractInfo() {
				...
				that.$api('contract.getAllContractList', params).then(res => {
					if (res.code == 200) {
